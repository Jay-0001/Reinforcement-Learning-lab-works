...
ENVIRONMENTS = {
    ...
    "MountainCar-v0": {
        ...
        "policies": {
            ...
            "optimal?": lambda state: 1 if state[0] < -0.6 else 2
        },
    },
    "CartPole-v1": {
        "render_mode": "human",
...
def run_simulation(env_name, policy_name, num_steps=200, delay=0.05):
    env = gym.make(env_name, render_mode="human")
    policy = ENVIRONMENTS[env_name]["policies"][policy_name]
    
    print(f"{env.action_space} and {env.observation_space}")
...
policy_name = "optimal?"  # Change to another policy name as needed
